...
- [ ] **Taxonomy integration — schema, classifier wrapping, enrichment job, payload extension, Zone A.5 activation, tests** — Three-session build that wires the taxonomy classifier into the live product. (a) Schema v15 migration adding `category`, `category_confidence`, `category_assigned_at` columns to `known_wallets`; (b) extract the existing classifier function (already callable internally at `classify_agents_taxonomy.py:95`) into an importable module with a `classify_from_stored_scan()` variant that reads `scans.response_json` instead of refetching Blockscout; (c) add a 6th APScheduler enrichment job following the pattern at `api.py:1556-1602`, decoupled from the scan path; (d) one-time backfill run against the ~27K agent population; (e) add `category` and `category_confidence` to the `_build_board()` and `get_agent_public_profile()` SELECTs (see also Taxonomy Category in AHS API Response under AHM Intelligence for the `/ahs` endpoint variant); (f) replace the `'AGENT TYPE CLASSIFICATION COMING SOON'` placeholder at `agent.html:603` with a conditional pill that renders when category is non-null, silently omits when null (CSS class `.profile-agent-type` already defined); (g) tests covering classifier output, persistence, payload surface, omit-when-null behaviour. Sequencing: session 1 = a+b+e; session 2 = c+d+partial g; session 3 = f+remaining g. Gated by the taxonomy classifier validation entry above (validation must produce defensible accuracy number first) and the `response_json` coverage check entry below (sizes the backfill). Source: `ahm-internal/docs/taxonomy-integration-spike.md` section 7. First noted: Jun 6 2026 (taxonomy integration spike)
- [ ] **Taxonomy lookup table — refresh contract set and establish recurring maintenance cadence** — The 36-contract lookup table at `scripts/taxonomy_contracts.json` was last updated 2026-04-25. Covers major Base protocols (Uniswap, Aerodrome, Aave, Compound, Morpho, Chainlink, Zora, EAS, Seaport) but misses any protocols deployed or gaining agent traction since April. The 24.3% unclassifiable rate from the POC will be HIGHER on the current population if new protocols have emerged — and likely much higher on the long-tail D-grade cohort. This is RECURRING maintenance, not a one-time refresh. Define a cadence (quarterly? on protocol launch?) and a source of truth for the contract set (manual research, DeFi Llama, Base protocol registry?). Refresh the table as part of (or before) the taxonomy integration build to reduce the visible-omission rate at launch. Source: `ahm-internal/docs/taxonomy-integration-spike.md` section 8, risk 1. First noted: Jun 6 2026 (taxonomy integration spike)
...
- [ ] **Taxonomy classifier — refactor fragile ACP reclassification heuristic** — The classifier has special-case ACP handling at `classify_agents_taxonomy.py:129-153`: ACP agents default to "Orchestration Agents", then get reclassified if subcategory signals (hardcoded 3-signal set: dex, oracle, nft_media) dominate. This logic breaks silently as ACP agent behaviour diversifies or the taxonomy expands. Lower priority than the other taxonomy entries — won't bite until ACP agents broaden beyond the current 3 signal sets — but worth marking now so it doesn't get lost. Either refactor to a general signal-set framework or replace with the same classification path as non-ACP agents (see also Issue #140, wallet-primitive bucket entry in this section — related but distinct: #140 is about the classified/wallet-only/unclassifiable three-bucket model, this entry is about the ACP-specific reclassification logic). Source: `ahm-internal/docs/taxonomy-integration-spike.md` section 8, risk 6. First noted: Jun 6 2026 (taxonomy integration spike)
