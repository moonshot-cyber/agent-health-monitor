...
- [ ] **Register Reclaim developer app (app ID + secret)** — Required to define custom CEX-balance providers for real zkTLS proof generation against AHM provider definitions. Blocks any real proof generation beyond the test fixtures used in the spike. Pablo-only action — not automatable. Cross-references: zkTLS spike (Completed Jun 3 2026), production item 8 in `docs/zktls_spike.md`. First noted: Jun 3 2026 (zkTLS spike).
...
- [ ] **zkTLS as off-chain trust signal input** — AHM scores on-chain wallet behaviour; agents with legitimate off-chain trust context (CEX holdings, verified infrastructure, credentials) cannot surface it without sacrificing privacy, so the score under-credits legitimate agents whose trust signals live off-chain. The INSUFFICIENT-confidence verdict on Job #3 is the concrete motivating case — sparse on-chain history does not mean low trustworthiness. zkTLS proofs could bridge this: an agent proves a claim ("off-chain balance exceeds X", "running on verified infrastructure Y") without revealing underlying data; the proof is independently verifiable and composes alongside on-chain signals rather than replacing them. Open architectural questions (unresolved, not yet taken public): does this fit ERC-8183's hook architecture as a beforeAction signal a relying party could optionally require; or does it belong at the ERC-8210 / ERC-8240 layer; or somewhere new. Not on build path. Natural home for first discussion is the ALIA Section 5 calibration co-spec (off-chain/on-chain signal composition is its subject) rather than a public ERC thread mid-audit-lock-convergence. Revisit trigger: when ALIA Section 5 drafting is active, or if an integrator explicitly raises off-chain signal support. First noted: May 20 2026.
  - Spike completed Jun 3 (PR #189) — no longer pure research; see Completed entry under Completed Jun 3 2026. Production path enumerated in `docs/zktls_spike.md`.
  - Infra maturity (Jun 3): verifiable-privacy primitives crossing into table stakes — Brave routing Leo to NEAR AI TEEs, Apple PCC normalising hardware-attested inference, Venice shipping per-request TEE/E2EE at consumer scale. Attestation infra no longer the gating uncertainty.
  - ALIA Section 5 route contingency: the entry above names ALIA Section 5 co-spec as natural first discussion home, but May 2 verification audit findings (oracle.alia.network dead, one claim contradicted) make that route contingent on ALIA re-verification.
- [ ] **AHM canonical methodology document in project knowledge** — Project knowledge currently lacks a single authoritative source for product methodology (D1-D4 definitions, AHS scoring, confidence schema, version markers). Claude/CC sessions have been inferring methodology from chat-derived summaries and userMemories, which has produced unverified framings (e.g. "D3 cross-registry composition" propagated into ALIA Section 3.2 without grounding in code). A canonical `ahm-methodology.md` sourced from code and product decisions — not from prior session summaries — would let future sessions read methodology first and avoid generating drift. Should cover: dimensional definitions (D1, D2, D3, D4), AHS scoring formulae and weights, confidence schema, version markers, what each component does NOT measure. First noted: May 21 2026
- [ ] **Cross-registry composition as candidate new AHM dimension** — AHM already ingests multiple registries (ACP/Virtuals, Olas, Celo, Arc, ERC-8004, CardZero); an agent's appearance across multiple registries is a distinct trust signal from any single-registry presence and from infrastructure health (current D3). Worth considering as a future dimension (D5 or successor) rather than overloading D3. Motivating context: this framing was used pre-publication in ALIA Section 3.2 and Raul's blog draft, was corrected to match code-canonical "D3 = Infrastructure Health". The signal genuinely exists but isn't currently surfaced as a scored dimension. Not on build path. Revisit trigger: when an integrator explicitly raises cross-registry trust signals, or when overall dimensional architecture review happens. First noted: May 21 2026
- [ ] **D4 historical outputs as retrospective trust signal feeding back into composite AHS** — D4 currently operates post-transaction via AHM Verify, evaluating output correctness against a task spec. Idea: aggregate historical D4 outputs per agent over time as a retrospective behavioural signal (e.g., "this agent has produced N evaluated outputs, M% of which scored above quality threshold X"), then fold that aggregate back into the pre-transaction AHS as a new dimension or as a modifier to existing dimensions. This would close a structural gap — currently AHS scores future trust from past on-chain behaviour but not from past output-quality verification. The two product surfaces (AHS pre-transaction, Verify post-transaction) would compose into a single trust signal over time. Open architectural questions: how to weight historical D4 signals against more recent on-chain behaviour; whether D4 history should be a new dimension (D5) or a modifier on existing dimensions; how to handle agents with zero D4 history. Not on build path. Revisit trigger: when AHM has paying integrators using both AHS and Verify, producing enough D4 evaluation history to make aggregation meaningful. First noted: May 31 2026
- [ ] **Hardcoded ecosystem stats drift detection — idea-stage** — The May 31 2026 documentation audit identified four inconsistencies across documentation sources, the largest being agent count terminology drifting between hardcoded stats in public docs / READMEs / Overview.md and the live `AHM_VALUE_PROPOSITIONS.md` source-of-truth. Candidate mitigation: automated CI step running a custom grep script against the repo, flagging any numeric ecosystem-stat values (agent counts, average AHS, registry counts, etc.) that appear outside `AHM_VALUE_PROPOSITIONS.md` and aren't whitelisted as intentional point-in-time snapshots. Not committing to build yet — current drift was caught manually by audit; if drift recurs on next audit despite that catch, promote this entry to a concrete P3 todo and scope the script. Decision gate: re-examine when next documentation audit runs. First noted: Jun 6 2026
...
- [x] **SUPERSEDED: D4 Output Quality Score (concept, Phase 2)** — Potential future AHS dimension derived from aggregated post-transaction output quality verdicts across an agent's job history. If an agent consistently produces low-quality deliverables that get rejected by content verification evaluators (e.g. ThoughtProof-style DISSENT/HOLD verdicts), that rejection pattern should feed into their AHS score as a fourth dimension alongside D1 Solvency, D2 Behavioural Consistency, D3 Operational Stability. Would require either: (a) a data-sharing agreement with content verification providers, or (b) reading rejection patterns from on-chain ERC-8183 job history. No action until D3 is live and ERC-8183 mainnet job volume is measurable. Source: ThoughtProof research, Apr 7 2026. **Superseded by "D4 historical outputs as retrospective trust signal" entry (May 31 2026).** D4 is now live as the AHM Verify product surface (multi-adjudicator output verification, post-transaction). The open question has evolved from "should we build D4" (this entry's framing) to "should historical D4 outputs feed back into AHS as a retrospective trust signal" (new entry below).
...
- [ ] **AHM Shield live status surface audit** — Shield is now the only marketed product (of Monitor / Shield / Verify / Intelligence) without a live product surface. Verify revival (May 17 2026) closes the verify gap. Shield SDK is on the parked list. Risk: any public framing assuming the four-product structure reads as hollow on the Shield surface specifically — most recent example: ALIA Section 3.2 (sent to Patrick May 17 2026) implicitly references the suite. Decision needed: (a) build minimal Shield "coming soon" landing page protecting the marketing surface without re-opening the product; (b) drop Shield from the marketed four until a real build commitment; (c) leave as-is and accept the risk. Tie this to the AHM Unlimited pricing decision and any upcoming public roadmap refresh (see both entries in this section). First noted: May 17 2026
...
- [ ] **AHM Unlimited pricing decision** — Decide whether the $99/mo Unlimited tier should include D4 (AHM Verify) verdicts or keep them separately priced. Currently zero Stripe conversions across all tiers, so decision is not urgent but should be made before any pricing refresh. **Status update May 17 2026:** Decision now actionable — Verify (D4) is live and paywalled at $0.50 standard / $1.50 deep per verdict as of May 17 2026. No longer deferrable if pricing-page refresh or public roadmap update is planned. Considerations: (a) including D4 in Unlimited raises LLM cost exposure per subscriber and creates a soft cap risk; (b) keeping D4 separately priced preserves pay-per-verdict revenue model but adds pricing complexity. Resolve before next public roadmap or pricing-surface update (see AHM Shield live status surface audit entry under P3 → Other). Source: chat history
...
- [ ] **Intelligence subdomain agent count out of sync with main dashboard** — As of May 4 2026, intelligence.agenthealthmonitor.xyz shows ~10k agents scanned while the main dashboard at agenthealthmonitor.xyz shows ~16k. The Layer 1 KPI dashboard architectural intent (AHM Intelligence — Public KPI Dashboard & Agent Taxonomy → Layer 1 — v1 KPI Dashboard) was "nightly snapshot updates" but the snapshot job has either stopped running or stopped picking up the latest scan totals. Visible production gap on a credibility-positioning surface — Don Gossen and standards-track readers landing on the intelligence page see a smaller number than the main site claims. Action: investigate the snapshot job (Railway service running it, schedule, last successful run timestamp); fix the breakage or replace with a live count query against the same source the main dashboard uses; verify nightly cadence resumes. First noted: May 4 2026 (visible production drift, user-spotted)
- [ ] **Backlog drift between repo and project-knowledge — widen lockstep rule** — Discovered May 4 2026 during the May 2 reconciliation. PRs #149 (May 2) and #150 (May 3) had landed on master between the project-knowledge upload and the May 4 reconciliation; the project-knowledge copy of ahm_backlog.md was never refreshed, so the reconciliation was scoped against a stale backlog. Result: 1 pure-duplicate diff (Diff 4) and 1 partial-duplicate diff (Diff 8b) had to be cleaned up post-PR via amended commit + force-push. The backlog-update skill's lockstep rule covers the skill itself drifting between repo and project knowledge but doesn't cover ahm_backlog.md drifting. Two complementary fixes: (a) widen the session-start lockstep check to enumerate git log --since=<last-upload> -- ahm_backlog.md and audit each commit before scoping any reconciliation; (b) add backlog re-upload to a post-merge checklist whenever any backlog-touching PR lands. Both should land as a small PR against the skill itself, not just a backlog entry. P2-priority for skill correctness — track here for visibility, action via skill PR. First noted: May 4 2026 (post-mortem of May 2 reconciliation overlap with PR #149/#150)
- [ ] **CC autonomy ceiling for backlog work** — During the May 2 reconciliation session, CC twice acted on suggested cleanup before user authorisation (offered surgical dedupe before line numbers were verified; force-pushed amended commit before surgery was reviewed) and twice misattributed file content with confident-sounding explanations that didn't survive raw-content verification. Mechanical application of pre-decided diffs is fine. Anything involving deletion of existing repo content, dedupe judgement, force-push, or amend needs explicit "proceed" before action. Capture in CC handoff template (skills/backlog-update/references/ or similar): "STOP after diagnostic / proposed action; wait for explicit authorisation before deleting, amending, or force-pushing." Land alongside the lockstep-widening skill PR above. First noted: May 4 2026
- [ ] **Stale P1 outreach cleanup — PayAI, Bankr, Daydreams** — dedupe_check.py --audit --threshold 0.55 flagged 25 cross-section pairs during skill build (May 3 2026), highest-priority subset being P1 outreach items in Pending Response / Follow-up (Mar 16) whose follow-up DM is already in the Completed section. Specifically: PayAI (P1 line / Completed line), Bankr Skills PR #195 (P1 / Completed), Daydreams (P1 / Completed). For each: judgement is whether the original outreach is genuinely closed (mark P1 entry complete or remove) or whether the Completed line covered only the DM and a substantive follow-up is still pending (leave P1 alone, possibly update with chase status). Single-edit work, three small decisions. First noted: May 3 2026 (skill-build dedupe audit run)
- [ ] **Local + remote branch cleanup at scale** — git branch -a on May 3 2026 in agent-health-monitor showed ~120 local branches and ~150 remote-tracking branches, vast majority squash-merge artefacts already on master. May 2 handoff identified 4 confirmed-safe Phase A branches but actual debt is two orders of magnitude larger. Cleanup approach: (1) for each local branch, run git log master..<branch> — empty output = content already on master, safe to delete with git branch -D <branch> (capital D needed because squash-merge changes commit hash); (2) prune remote-tracking refs that no longer exist on origin via git fetch --prune; (3) branches still on origin can be deleted via GitHub UI bulk-delete or git push origin --delete <branch>. Tackle in batches by prefix (chore/, docs/, fix/, feat/) over multiple sessions rather than one big sweep. Low-priority hygiene. First noted: May 3 2026 (skill PR session branch list)
- [x] **Clean `agent_name` column on `known_wallets`** (completed May 2026) — Added a dedicated `agent_name` TEXT column to `known_wallets`, populated on ingestion and backfilled from existing `label` values for ACP and ERC-8004 rows. Build-blocking prerequisite for the agents surface leaderboard (see Agents Surface entry under AHM Intelligence). Schema v11, PR #161. First noted: May 20 2026 (Agents Surface design chat)
- [ ] **Doc correction — remove phantom "D5" references** — AHM is a four-dimensional model (D1 Wallet Hygiene, D2 Behavioural Consistency, D3 Infrastructure Health, D4 Output Quality). No D5 exists or is planned. `section-taxonomy.md`, the agents-surface design brief, and any parked "D3/D4/D5 expansion" backlog item all reference a non-existent D5 — correct them. First noted: May 20 2026 (Agents Surface design chat)
- [ ] **`prompts/cc-runs/zktls-spike-results.md` tracked on master despite gitignore** — `prompts/cc-runs/` is gitignored but `zktls-spike-results.md` was force-added during the spike session and carried through the squash merge of PR #189. Decide: `git rm --cached` in a future PR to remove from tracking, or accept deliberately as an exception. One-line hygiene item so this does not become silent precedent for force-adding other gitignored files. First noted: Jun 3 2026.
- [ ] **Operator-fleet grouping v2 — exact detection via registry_owner** — v1 of operator-fleet grouping (shipped: collapses fleets on the Healthiest and Most Consistent boards) uses a name heuristic for detection — a two-layer rule (shared 15+ char name prefix, plus a strictly-lowercase `^[a-z]+agent[a-z]*$` pattern, min 5 members). Works on the four known fleets (Zyfai, UniAgent, ClawWork, lowercase stem+agent) with near-zero false positives, but is inherently fragile: will silently miss future fleets that do not fit either name shape (e.g. CamelCase fleet, or names sharing fewer than 15 chars). For a behavioural-diagnostics product, name-pattern matching is not an acceptable permanent detection mechanism. v2 replaces the heuristic with exact operator detection using the ERC-8004 registry owner (`ownerOf(agent_id)`). The owner is already fetched on-chain during scanning in `erc8004_scan.py` but is currently discarded (written only to a local CSV, never persisted). Backend work required (~50-80 lines): (a) schema migration adding `registry_owner TEXT` column to `known_wallets`; (b) store `source_agent_id INTEGER` (the agent_id → owner → wallet_address link is currently lost after scanning); (c) persist `registry_owner` in `erc8004_scan.py` `scan_wallets()` / `log_scan()`; (d) surface `registry_owner` in `_build_board()` output; (e) backfill existing agents via re-run of ERC-8004 scan or a one-off `ownerOf()` script. Frontend change is minimal: v1 isolated detection into a separate `detectFleets()` function specifically so v2 can swap it for a single `GROUP BY registry_owner` (`HAVING COUNT >= 5`) with no change to the grouping/render mechanics. **Unpark trigger:** pick up when the v1 name heuristic visibly misses a fleet, when any false-grouping is reported, or when Cleanest-board grouping is added. First noted: May 24 2026
- [ ] **Taxonomy classifier — build labelled evaluation set and run validation before integration** — The taxonomy classifier (`scripts/classify_agents_taxonomy.py`) has never been validated against human-assigned categories. The 75.7% figure from the April 2026 POC is COVERAGE (got-a-category rate), not ACCURACY (correct-category rate). Before integrating per-agent taxonomy classification into the product, build a labelled evaluation set of ~100 agents stratified to MIRROR PRODUCTION POPULATION DISTRIBUTION (not top-of-leaderboard skew — most of the population is long-tail D-grade cold wallets, and that's where the classifier will struggle most). Write a labelling spec defining decision rules for ambiguous cases (multi-category agents, low-activity agents, card-vs-behaviour mismatches) before labelling starts to ensure consistency. Crucially, the spec must define DEFENSIBILITY THRESHOLDS for the HIGH/MEDIUM/LOW confidence labels — the classifier currently assigns HIGH on a >50% interaction ratio with no minimum sample-size requirement, meaning an agent with 2 transactions both to Uniswap gets "HIGH confidence Financial." Decide what minimum activity is required for HIGH to be honestly displayed. Run the classifier against the labelled set, produce a real accuracy number and confusion matrix, then decide whether to proceed to integration (see taxonomy integration build entry below). Gates the taxonomy integration build. Source: `ahm-internal/docs/taxonomy-integration-spike.md` sections 1 (confidence model) and 8 (risks 2, 3). First noted: Jun 6 2026 (taxonomy integration spike)
- [ ] **Taxonomy integration — schema, classifier wrapping, enrichment job, payload extension, Zone A.5 activation, tests** — Three-session build that wires the taxonomy classifier into the live product. (a) Schema v15 migration adding `category`, `category_confidence`, `category_assigned_at` columns to `known_wallets`; (b) extract the existing classifier function (already callable internally at `classify_agents_taxonomy.py:95`) into an importable module. Note: the original spike proposed a `classify_from_stored_scan()` variant reading `scans.response_json` to avoid Blockscout refetching — DB inventory on Jun 7 confirmed this is not possible (`response_json` and `shadow_signals_json` both contain only computed scores, not raw transaction data). The classifier must fetch fresh from Blockscout for every agent; (c) add a 6th APScheduler enrichment job following the pattern at `api.py:1556-1602`, decoupled from the scan path; (d) one-time backfill run against the ~133K-agent population (per the Jun 7 response_json coverage check, full population is 133,184 agents). Must re-fetch from Blockscout for every agent: ~8 hours at 0.22s rate limit for `txlist` alone; ~16 hours if also fetching `tokentx` per Step 4 of `ahm-internal/docs/taxonomy-first-look-refined.md`. Backfill runtime is a real consideration: rate-limit risk on Blockscout free tier over 8-16h sustained traffic; the enrichment job design (part c) should support resumability and batched checkpointing; (e) add `category` and `category_confidence` to the `_build_board()` and `get_agent_public_profile()` SELECTs (see also Taxonomy Category in AHS API Response under AHM Intelligence for the `/ahs` endpoint variant); (f) replace the `'AGENT TYPE CLASSIFICATION COMING SOON'` placeholder at `agent.html:603` with a conditional pill that renders when category is non-null, silently omits when null (CSS class `.profile-agent-type` already defined); (g) tests covering classifier output, persistence, payload surface, omit-when-null behaviour. Sequencing revised after sniff test: lookup-table redesign (see merged entry below) → re-run sniff test to confirm improved coverage → validation against labelled set (entry above) → integration build. The eventual ship is v1 — contract-pattern based, with an honest accuracy number — not the eventual product (see behavioural classification engine entry under Future Product Concepts). Gated by the taxonomy classifier validation entry above (validation must produce defensible accuracy number first), the lookup-table redesign entry below (must land before validation), and the backfill runtime planning implied by part (d) above (8-16h Blockscout fetch with resumability requirements). Source: `ahm-internal/docs/taxonomy-integration-spike.md` section 7; sniff-test findings at `ahm-internal/docs/taxonomy-first-look.md`. First noted: Jun 6 2026 (taxonomy integration spike)
- [ ] **Taxonomy classifier v1 — lookup-table redesign + ACP default replacement (must land before validation phase)** — The Jun 6 sniff test (`ahm-internal/docs/taxonomy-first-look.md`) against 24 production leaderboard agents revealed structural issues that require a lookup-table redesign, not just a refresh. Key findings and required actions: (1) **Identity Registry exclusion or re-categorisation:** the ERC-8004 Identity Registry contracts (`0x8004a169…`, `0x8004baa1…`) produce false "Identity & Trust" classifications on 13/21 agents because every ERC-8004 agent registers itself — this is a registration artefact, not behavioural signal. Exclude from classification or recategorise as "Infrastructure" so it doesn't dominate. (2) **0x Allowance Holder identification (CORRECTED — not Permit2 as previously suspected):** contract `0x0000000000001ff3684f28c67538d4d072c22734` is confirmed via Basescan as **0x: Allowance Holder** — the 0x Protocol's swap / allowance contract. Accounts for 153/250 unmatched interactions across 6 agents. Add to Financial category (DEX aggregation). Behavioural implication: the Verification-cluster agents (azoagenttx, sevoagentux, milaagenty, deryagent, meroagentx) doing 27-32 interactions each with this contract are performing DEX swaps via 0x — they are DEX users, not Verification agents. Their current Verification classification on 3 EAS attestations is dominated by the unmatched 0x-swap activity. Adding this contract to the lookup table will likely flip 5 of the 21 classifications from Verification to Financial. (3) **Top-10 unmatched contracts triage:** the sniff test surface (Section 4 of the report) lists 20 unmatched contracts ranked by interaction count. Triage the top 10 for lookup-table inclusion. (4) **ACP default replacement:** the classifier currently assigns HIGH-confidence "Orchestration Agents" to any ACP agent with no contract matches (`classify_agents_taxonomy.py:129-153`). The sniff test showed prosciutto classified as Orchestration/HIGH on zero evidence. Replace ACP default with UNCLASSIFIABLE-when-no-evidence — HIGH confidence on zero behavioural data is misleading. (5) **Recurring maintenance cadence:** still applies beyond first-pass redesign. Define a cadence (quarterly? on protocol launch?) and a source of truth for the contract set. The 36-contract table was last updated 2026-04-25; the 24.3% unclassifiable rate from the April POC is likely worse on the current population. **Gates the validation phase** (taxonomy classifier validation entry above) — the labelled evaluation set must be validated against a redesigned lookup table, not the current one. Source: `ahm-internal/docs/taxonomy-integration-spike.md` section 8 (risks 1, 6); `ahm-internal/docs/taxonomy-first-look.md` (sniff test). First noted: Jun 6 2026 (taxonomy integration spike)
- [x] **Scans `response_json` coverage — one-line read-only check to size the taxonomy backfill** (completed Jun 7) — Run a single SELECT to determine what fraction of `scans` rows have `response_json` populated. The taxonomy enrichment job (see taxonomy integration build entry above, part c) plans to reuse stored scan data rather than refetch Blockscout — if coverage is high, the full 27K backfill takes ~1.4 minutes; if coverage is low and the job falls back to live Blockscout fetches, the same backfill takes ~5.3 hours. Trivial standalone task — `SELECT COUNT(*) FROM scans WHERE response_json IS NOT NULL` plus the same against the agent population joined to `known_wallets`. Should be done before the taxonomy integration build session 2 (the backfill session) so the duration estimate is real, not speculation. Outcome (Jun 7 2026): production query confirmed 100% coverage — 133,184 of 133,184 scans rows have `response_json` populated. The taxonomy backfill is the ~1.4-minute case (reuse stored scan data), not the ~5.3-hour case (live Blockscout fetches). Outcome addendum (Jun 7 2026 — same day): although `response_json` coverage is 100%, follow-up DB inventory revealed the column contains only final scores (`{ahs, d1, d2, grade}`), NOT raw Blockscout transaction data. The `shadow_signals_json` column was also checked and contains only D2 session-continuity metrics, not raw data either. The "~1.4-minute backfill" conclusion was therefore wrong — the backfill must re-fetch from Blockscout for every agent. See entry 206 (taxonomy integration) for revised estimate. Source: `ahm-internal/docs/taxonomy-integration-spike.md` section 8, risk 4. First noted: Jun 6 2026 (taxonomy integration spike)
- [ ] **Taxonomy schema — decide whether to persist `category_scores` or only `primary_category`** — The classifier returns a `category_scores` dict (raw interaction count per category) alongside the single `primary_category` chosen by majority vote. The proposed v15 schema (see taxonomy integration build entry above, part a) persists only `primary_category` — the full distribution is discarded. An agent that genuinely operates across categories (e.g. a DeFi agent that also does oracle reads) gets a single label that may misrepresent it. Open decision: add `category_scores_json` to the v15 migration now, preserving future flexibility for "Financial + Intelligence" combined-label presentations and for per-category-confidence displays — or defer until a concrete use case forces it. Decision needs to be made BEFORE the schema migration lands, as retrofitting later is a v16 migration. Source: `ahm-internal/docs/taxonomy-integration-spike.md` section 8, risk 5. First noted: Jun 6 2026 (taxonomy integration spike)
- [ ] **Crawler observatory `/up` health check filtering decision** — The crawler observatory currently captures `/up` health check pings alongside genuine crawler traffic, polluting the dataset. Decide on a filtering approach: (a) exclude `/up` requests at ingestion time (lose health-check signal but cleaner crawler analytics); (b) tag `/up` requests with a `request_type` field so they're filterable downstream (preserves signal, requires schema migration); (c) ignore at query time only (simplest, but every query needs to remember to exclude). Decision affects schema. Should be made before further crawler observatory work compounds the dataset issue. First noted: Jun 6 2026
- [ ] **Crawler observatory `crawler_hits` table retention/cleanup mechanism** — The `crawler_hits` table grows monotonically with no retention policy. At current ingestion rate this will eventually become an operational issue (Railway volume cost + query performance degradation). Decide on retention: (a) hard cap by row count (e.g. keep last 1M rows); (b) time-based (e.g. drop rows older than 90 days); (c) tiered (recent raw, older aggregated into daily summaries). Hand-in-hand with the `/up` filtering decision above — different retention is sensible for genuine crawler traffic vs health-check noise. Add APScheduler job for whatever policy lands. First noted: Jun 6 2026
- [ ] **Move `prompts/composed-evaluators-grounding-pass-results.md` into `prompts/cc-runs/`** — Minor file location inconsistency. Other CC investigation outputs live in `prompts/cc-runs/`; this one is one directory level above. Move via `git mv` to align. Single-edit hygiene fix. First noted: Jun 6 2026
- [ ] **Foundations doc Section 8 fix — point to ahm-docs/public/ not agent-health-monitor/docs/AHM_Overview.md** — `foundations-doc-draft.md` Section 8 currently references `agent-health-monitor/docs/AHM_Overview.md` as the canonical pointer for public-facing overview content. That file is gitignored and not the authoritative public source. Fix: edit Section 8 to point to `ahm-docs/public/` instead. Apply to the project-knowledge copy of the foundations doc (source-of-truth for chat consumption). Small concrete todo. First noted: Jun 6 2026
- [x] **snapshot.json appears to carry only `healthiest` board — investigate snapshot generator vs live API discrepancy** (completed Jun 7) — During the taxonomy sniff test (Jun 6 2026), the production snapshot at `https://intelligence.agenthealthmonitor.xyz/data/snapshot.json` was observed to carry only the `healthiest` board. The production live API at `https://agenthealthmonitor.xyz/api/leaderboard` returns all three boards (`healthiest`, `cleanest`, `consistent`), 500 entries each. If `intelligence.agenthealthmonitor.xyz/agents` renders Cleanest Wallets and Most Consistent boards from snapshot.json (per the leaderboard build), this is a user-visible regression on a live surface — those boards may currently be empty or stale. Investigated: (a) confirmed no discrepancy — both endpoints carry all three boards with 500 entries each; (b) `intelligence.agenthealthmonitor.xyz/agents` reads from snapshot.json exclusively (no live API fallback); (c) the generator produces all three boards in both `--db` and `--api` modes; (d) no fix needed. Outcome (Jun 7 2026): false alarm. Investigation report at `ahm-internal/docs/snapshot-regression-investigation.md` confirms the Jun 6 committed snapshot (commit 71aee30) carries all three boards with 500 entries each. The original observation was caused by WebFetch truncating the 1500-entry JSON payload and surfacing only the `healthiest` board. No regression. No code fix needed. Cross-references: "Intelligence subdomain agent count out of sync with main dashboard" (related intelligence-surface sync issue, same section); `ahm-internal/docs/taxonomy-first-look.md` (discovery moment). First noted: Jun 6 2026 (taxonomy sniff test)
...
- [x] **`agentURI` resolution for Celo and Arc scanners** (completed May 2026) — `celo_scan.py` and `arc_scan.py` now resolve the `agentURI` captured from the `Registered` event and populate `agent_name`, closing the largest registry naming gap (~7,188 Celo wallets). URI resolution helpers lifted into a shared `uri_utils.py` module reused by ERC-8004. PR #162. First noted: May 20 2026 (Agents Surface design chat)
...
- [ ] **Behavioural classification engine — longitudinal intent inference (commercial product concept, multi-month scoping)** — The taxonomy v1 classifier (contract-pattern matching against a lookup table) ships a per-agent type label on profile pages. v2 is the inverse: behavioural inference from observed on-chain activity over time, making self-declared metadata (Olas categorisation, ERC-8004 metadata, Virtuals ACP roles) a thing to verify against rather than trust. This aligns with AHM's "named-issuer accountability" DNA — every other agent-taxonomy effort treats classification as self-declared and trusted; AHM derives type from behaviour. The AHM-specific advantage is the longitudinal scanning time series: classification + longitudinal = drift detection (agent pivots, repurposing, stated-vs-observed divergence, category instability as a behavioural anomaly that feeds back into D2 scoring). Three use modes: (1) per-agent type that's defensible — v2 of Zone A.5; (2) drift signal that feeds back into AHM scoring — internal; (3) standalone classification API for other infrastructure — separately-positioned commercial product, potentially licensed. Commercial framing: hidden by default, potentially licensed or API'd, distinct from the v1 contract-matching ship (v1 is the visible free pill on profile pages; v2 is the hidden commercial product — open architectural question whether v2 eventually replaces v1 or powers it from behind). Scoping work before any build commitment: (a) strategy doc articulating the wedge, dependencies, v1-vs-v2 relationship; (b) data-availability spike — what longitudinal behavioural-pattern data does AHM persist today, what would it need to persist for v2, what's the capture cost; (c) methodology spike across candidate approaches (unsupervised clustering on feature vectors, supervised classification against labelled set, LLM-on-transaction-history, enriched rule-based). Honest characterisation: "scope and decide whether to invest" — NOT "build this". Months-out commercial bet, not a feature. Cross-references: Agent Intent Detection entry under P2 → Research / Open Concepts (drift-from-declared-purpose is one component of this broader concept); `ahm-internal/docs/taxonomy-first-look.md` (sniff test where the v1-vs-v2 distinction crystallised). First noted: Jun 6 2026 (taxonomy sniff test)
...
#### Agents Surface & Public Leaderboard — buildable spec complete
- Public agent discovery surface on `intelligence.agenthealthmonitor.xyz/agents`, headline feature a public leaderboard. Promotes the "Individual agent leaderboards" item deferred to v2 under Layer 1 above into a scoped, buildable design. Triggered by BaseHawk (criptic.io) surfacing as a direct leaderboard competitor.
- Design spec produced May 20 2026 (`ahm-agents-surface-spec.md`). Key locked decisions: four curated leaderboard boards (Healthiest / Cleanest Wallets / Most Consistent / Rising) rather than a single board with a dimension selector; boards addressed by `?board=` query param so `/agents/<x>` stays unconditionally an agent slug; D3 and D4 are not boards (D3 is `agent_url`-gated, D4 lives in ahm-verify) — they surface as per-agent detail only; negative-score-movement policy locked (leaderboard ranks but does not editorialise downward movement, Rising board has no Falling mirror, X engagement motion is positive-only); claim verification via `personal_sign` of a nonce, no gas, no on-chain tx; claimed agents get an "Operator-claimed" badge; naming convention locked to single-word noun labels for future-surface compatibility.
- Coupled to off-chain agent scanning (PR #154) at one point only — the claim flow, which is the sole naming path for off-chain agents. Stay coordinated with PR #154 on the claim flow; scanning-pipeline scope stays with PR #154.
- Build-blocking prerequisites: agent_name column (PR #161, merged) and Celo/Arc URI resolution (PR #162, merged) both complete. See P3 and P4 entries.
- First noted: May 20 2026 (Agents Surface design chat)
...
### zkPass (zkpass.org) — COMPETITIVE OVERLAP + CANDIDATE PROVIDER #2

- Repositioned around agents: "proof for humans, credit for agents", agent SDK, $ZKP token live. Direct overlap with AHM trust-scoring territory via their "AI Agent Trust Layer" messaging and TransGate SDK for verifiable credentials.
- Three-party TLS (3P-TLS) model (vs Reclaim's proxy model). On-chain Solidity verification contract — more mature verification surface than Reclaim's centralised attestor API.
- Dual classification: competitive-overlap watch item AND candidate provider #2 alongside Reclaim. A production design would abstract behind a provider-neutral `verify_attestation(proof, provider="reclaim"|"zkpass")` interface.
- Cross-references: zkTLS research entry under P2 → Research / Open Concepts; zkTLS spike (Completed Jun 3 2026); `docs/zktls_spike.md` Provider #2 Note.
- Action: Monitor zkPass agent SDK evolution and $ZKP token traction. Do not engage publicly. Revisit as integration candidate when AHM's Reclaim verification path reaches production.
- First noted: Jun 3 2026 (zkTLS spike).
...
- [x] **zkTLS feasibility spike** (completed Jun 3) — PR #189. `verify_reclaim_proof()` using manual EIP-191 signature recovery (chosen over official Reclaim Python SDK: sync API, fewer deps, no runtime attestor fetch) + `apply_attestation_confidence()` (verified solvency attestation lifts INSUFFICIENT → LOW only; never touches D1-D4 scores; never lifts above LOW). 35 tests including context-tamper negative test. Key finding: identifier recomputation via `json-canonical` (RFC 8785) matches real SDK fixtures — hard requirement for production (OQ4 in `docs/zktls_spike.md`); context fields are bound to the signature only via the identifier hash. Production requirements enumerated in `docs/zktls_spike.md`. New dependency: `json-canonical>=2.0.0` (pure Python). Cross-references: zkTLS research entry under P2 → Research / Open Concepts; Reclaim developer app registration under P2 → New Endpoints / Features; zkPass CI entry under Competitive Intelligence.

### Completed May 17 2026

- [x] **AHM Verify revived from parked status** (completed May 17) — Railway `ahm-verify` service had network/custom domain config intact but no source repo connected — likely disconnected at some point between last commit (Apr 22 2026) and now. Action taken: reconnected `moonshot-cyber/ahm-verify` master branch in Railway → service auto-built and deployed. Verified live: `https://verify.agenthealthmonitor.xyz/health` returns 200; `POST /v1/outputs` returns 402 Payment Required with x402 challenge containing bazaar discovery extension. Cloudflare CNAME flipped back to proxied. x402 paywall confirmed enforcing — no free LLM-spend path open. Bazaar discovery extension live with realistic example metadata (post-PR-#4 merge). Auto-deploy via GitHub webhook confirmed working. D4 dimension is now a live, paid, independent product surface.
