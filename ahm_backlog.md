...
- [ ] **Scans `response_json` coverage — one-line read-only check to size the taxonomy backfill** — Run a single SELECT to determine what fraction of `scans` rows have `response_json` populated. The taxonomy enrichment job (see taxonomy integration build entry above, part c) plans to reuse stored scan data rather than refetch Blockscout — if coverage is high, the full 27K backfill takes ~1.4 minutes; if coverage is low and the job falls back to live Blockscout fetches, the same backfill takes ~5.3 hours. Trivial standalone task — `SELECT COUNT(*) FROM scans WHERE response_json IS NOT NULL` plus the same against the agent population joined to `known_wallets`. Should be done before the taxonomy integration build session 2 (the backfill session) so the duration estimate is real, not speculation. Source: `ahm-internal/docs/taxonomy-integration-spike.md` section 8, risk 4. First noted: Jun 6 2026 (taxonomy integration spike)
